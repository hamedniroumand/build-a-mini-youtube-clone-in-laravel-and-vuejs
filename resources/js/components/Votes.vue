...
<script>
    ...
    export default {
        props: ["default_votes", "entity_owner", "entity_id"],
       /* props: {
            default_votes: {
                type: Array,
                required: true,
        ...
        methods: {
            async vote(type) {
                ...
                if (__auth() && __auth().id === this.entity_owner) return alert("You cannot vote this item.")

                if (type === "up" && this.upvoted) return;

                if (type === "down" && this.downvoted) return;

                const { data } = await axios.post(`/votes/${this.entity_id}/${type}`)
                if (this.upvoted || this.downvoted) {
                    this.votes = this.votes.map(vote => {
                        if (vote.user_id === __auth().id) return data;
                        return vote;
                    })
                } else this.votes = [...this.votes, data]
